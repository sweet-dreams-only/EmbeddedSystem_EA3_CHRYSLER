...
attrib -r "..\utp\contract\*.h" >>logfile.txt
 ...
 set dcf=Ap_WIRInputQual
 set RTEGEN="..\..\..\Tools\AsrProject\Generators\Rte\dvrtegen"


set SWC=Ap_WIRInputQual


%RTEGEN% -d "..\autosar\%dcf%.dcf" -m %SWC% -g i -o "..\src" >>logfile.txt
%RTEGEN% -d "..\autosar\%dcf%.dcf" -m %SWC% -g c -o "..\utp\contract\Ap_WIRInputQual" >>logfile.txt